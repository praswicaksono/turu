...
type Config struct {
	...
	Plugis         []interface{}   `json:"plugins,omitempty"`
	PluginConfigs  []PluginConfig  `json:"plugin_configs,omitempty"`
	SSLS           []SSL           `json:"ssls,omitempty"`
	Consumers      []Consumer      `json:"consumers,omitempty"`
	GlobalRules    []GlobalPlugins `json:"global_rules,omitempty"`
	StreamRoutes   []StreamRoute   `json:"stream_rules,omitempty"`
	Protos         []Proto         `json:"proto,omitempty"`
	PluginMetadata []interface{}   `json:"plugin_metadata,omitempty"`
...
func (info *BaseInfo) Creating() {
	if info.ID == nil {
		info.ID = strutil.GenMd5(strutil.RandomCharsV3(16))
	} else {
		// convert to string if it's not
		if reflect.TypeOf(info.ID).String() != "string" {
			info.ID, _ = goutil.ConvTo(info.ID, reflect.String)
		}
	}
...
// swagger:model Route
type Route struct {
	BaseInfo        `json:",omitempty,inline"`
